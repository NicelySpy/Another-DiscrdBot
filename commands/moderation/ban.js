...
module.exports={
    ...
    run: async(bot,message,args)=>{
        ...
        if(!args[1])return message.channel.send(`Please specify a reason! You can't ban someone with out a reason, can you?`)
        if(!Reason) return message.channel.send(`Please specify a reason! You can't ban someone with out a reason, can you?`)
   //     if(!User.banable)return message.channel.send(`You can not ban this user, they may have a role higher then me or the same role as me.`)
        if(!message.member.permissions.has("BAN_MEMBERS"))return message.channel.send(`You do not have the BAN_MEMBERS permission!`)
        User.ban(Reason)
