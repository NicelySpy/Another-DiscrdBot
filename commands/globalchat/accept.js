  ...
  run: async (bot, message, args) => {
    if (bot.globalChat.guilds.get(message.guild.id))
      return message.channel.send(
        `There is already a call going on in this guild! Join the party over at <#${bot.globalChat.channels.get(
          message.guild.id
        )}>`
      );
    if (!args[0]) return message.channel.send(`You must specify the guild id!`);
    if (bot.globalChat.calls.get(args[0]))
      return message.channel.send(
    ...
    const originalGuild = bot.channels.cache
      .get(bot.globalChat.channels.get(args[0]))
